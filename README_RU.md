...
В HTML-шаблонах Go будут поставляться следующие версии библиотек:

- **htmx**: последняя непроизводственная версия с CDN;
- **hyperscript**: последняя непроизводственная версия с CDN;
- (_опционально_) **CSS-фреймворк**: последняя непроизводственная версия с CDN;

В режиме разработки будут использоваться только официальные CDN, которые
...
3. CLI запускает бэкенд проекта (`5000` порт) с помощью простой команды 
   `go run`.
...
созданное и поддерживаемое [Vic Shóstak][author_url] с 🩵 для людей и роботов.
