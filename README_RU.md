...
**net/http** (в качестве Go-бэкенда), **htmx** и **hyperscript**, доступно в 
...
> - Без какого-либо CSS-фреймворка (только стили для кода с примером).
...
|--------------------|-----------------------------------------------------------------------------|
| `default`          | Не использовать какой-либо CSS-фреймворк (только стили для кода с примером) |
...
2. CLI загружает минимизированные версии **htmx** и **hyperscript** (из 
   официальных и надежных CDN) в папку `./static` и размещает их в виде 
   разделенных тегов `<script>` (внизу тега `<body>`) в HTML-шаблоне Go 
   [`templates/main.html`][repo_main_layout];
3. (_опционально_) CLI загружает непроизводственную версию выбранного 
   **CSS-фреймворка** (из официальной и надежной CDN) в папку `./static` и 
   размещает ее в виде тега `<link>` (внизу тега `<head>`) в шаблоне Go HTML 
   [`templates/main.html`][repo_main_layout];
4. CLI запускает бэкенд проекта на выбранном порту с помощью простой команды 
   `go run`.
...
2. CLI загружает минимизированные версии **htmx** и **hyperscript** (из
   ...
   разделенных тегов `<script>` (внизу тега `<body>`) в HTML-шаблоне Go
   [`templates/main.html`][repo_main_layout];
3. (_опционально_) CLI подготавливает производственную версию выбранного 
   **CSS-фреймворка** (с помощью инструмента [Vite][vite_url]) и размещает 
   его в виде тега `<link>` (внизу тега `<head>`) в HTML-шаблоне Go 
   [`templates/main.html`][repo_main_layout];
