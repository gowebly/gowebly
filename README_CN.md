...
就是这样！🔥 在 Go HTML 模板中使用内置的 net/http 包（作为 Go 后端）、htmx 和 
hyperscript，就能制作出精彩的网络应用程序。
...
>
> - 后端使用 net/http 软件包；
> - 服务器端口为 `5000`，超时（秒）： 读取时为 `5`，写入时为 `10`；
> - 最新版本的 htmx 和 hyperscript；
> - 没有任何 CSS 框架（只有示例代码的默认样式）。
...
| CSS framework | 说明                                           |
|---------------|----------------------------------------------|
| `default`     | 不使用任何 CSS 框架（仅使用示例代码的默认样式）                   |
...
1. CLI 会验证配置，并将所有设置应用于当前项目；
2. CLI 会下载最小化版本的 htmx 和 hyperscript（来自官方和可信 CDN）到
   `./static` 文件夹，并将它们作为分隔的 `<script>` 标记（位于 Go HTML 模板
   [`templates/main.html`][repo_main_layout] 中 `<body>` 标记的底部）；
3. (可选) CLI 将所选 CSS 框架的非生产版本（从官方和可信 CDN 下载）下载到 `./static` 文件夹，并将其作为 
   `<link>` 标记（位于 `<head>` 标记的底部）放入 Go HTML 模板
   [`templates/main.html`][repo_main_layout]； 
4. CLI 通过简单的 `go run` 命令在选定的端口上启动项目的后端。
...
2. CLI 会下载最小化版本的 htmx 和 hyperscript（来自官方和可信 CDN）到
   `./static` 文件夹，并将它们作为分隔的 `<script>` 标记（位于 Go HTML 模板 
   [`templates/main.html`][repo_main_layout] 中 `<body>` 标记的底部）；
3. (可选）CLI（使用 [Vite][vite_url]工具）准备一个选定 CSS 框架的工作版本，并将其放置在 Go HTML 模板 
   [`templates/main.html`][repo_main_layout] 中的 `<link>` 标记（位于 `<head>` 标记的底部）；
